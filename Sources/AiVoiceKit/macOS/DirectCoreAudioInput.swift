// Sources/AiVoiceKit/macOS/DirectCoreAudioInput.swift
// Ported from FluidVoice/Sources/Fluid/Services/DirectCoreAudioInput.swift
// Changes: wrapped in #if os(macOS), updated queue label namespace.
#if os(macOS)
import CoreAudio
...
/// ring in C. This Swift wrapper drains that ring away from Core Audio's realtime
/// thread, so resampling, level calculation, logging, and ASR buffer mutation
/// never happen in the device callback.
final class DirectCoreAudioInput: @unchecked Sendable {
    ...
    private let workerQueue = DispatchQueue(
        label: "com.nerdsnipe.alric.audio.direct-input-consumer",
        qos: .userInteractive
    )
    ...
    func start() throws {
        ...
        guard !fv_core_audio_capture_is_running(capture) else { return }
    ...
    // MARK: - Realtime packet consumer (runs on workerQueue)
    ...
    private nonisolated static func consumePackets(
        ...
        while true {
            ...
            guard fv_core_audio_capture_is_running(capture) else {
                // AudioDeviceStop waits for the IOProc to leave. One final drain above
                // therefore captures the complete tail.
                return
            }
    ...
    private static func error(status: OSStatus, operation: String) -> NSError {
        NSError(
            ...
            code: Int(status),
            userInfo: [NSLocalizedDescriptionKey: "Failed to \(operation) (OSStatus \(status))."]
        )
    }
}
#endif
