// Sources/AiVoiceKit/Shared/HotkeyShortcut.swift
// Ported from FluidVoice/Sources/Fluid/Models/HotkeyShortcut.swift
// Changes: public access, #if os(macOS) guards around AppKit/Carbon, extended conflictsWith
//          for regular keyboard shortcuts.
...
public struct HotkeyShortcut: Codable, Equatable, Sendable {
    ...
    public private(set) var kind: ShortcutKind
    public var keyCode: UInt16
    public var modifierFlags: NSEvent.ModifierFlags
    public var modifierKeyCodes: [UInt16]
    public private(set) var mouseButton: Int?

    public enum CodingKeys: String, CodingKey {
        case kind, keyCode, modifierFlagsRawValue, modifierKeyCodes, mouseButton
    }
    ...
    public static func mouseButtonToString(_ button: Int) -> String {
        switch button {
    ...
    public static func characterForKeyCode(_ keyCode: UInt16) -> String? {
    ...
    public init(keyCode: UInt16, modifierFlags: NSEvent.ModifierFlags, modifierKeyCodes: [UInt16] = []) {
        self.kind = .keyboard
    ...
    public init(mouseButton: Int, modifierFlags: NSEvent.ModifierFlags) {
        self.kind = .mouse
        self.keyCode = 0
        self.modifierFlags = modifierFlags.intersection(Self.relevantModifierMask)
        self.modifierKeyCodes = []
...
extension HotkeyShortcut {
    public static let relevantModifierMask: NSEvent.ModifierFlags = [.function, .command, .option, .control, .shift]
    ...
    public static func normalizedModifierKeyCodes(from modifierKeyCodes: [UInt16]) -> [UInt16] {
    ...
    public var relevantModifierFlags: NSEvent.ModifierFlags {
        self.modifierFlags.intersection(Self.relevantModifierMask)
    }

    public var isMouseShortcut: Bool {
        self.kind == .mouse
    }

    public var isUnmodifiedLeftOrRightClick: Bool {
    ...
    public var normalizedModifierKeyCodes: [UInt16] {
    ...
    public var modifierTriggerFlag: NSEvent.ModifierFlags? {
        guard !self.isMouseShortcut else { return nil }
        return Self.modifierFlag(forKeyCode: self.keyCode)
    }

    /// Returns true when two shortcuts would race because one begins as the other's modifier prefix,
    /// or when two regular keyboard shortcuts map to the same key+modifier combination.
    public func conflictsWith(_ other: HotkeyShortcut) -> Bool {
        // Regular keyboard shortcuts conflict when they are identical.
        if !self.isModifierOnlyShortcut, !self.isMouseShortcut,
           !other.isModifierOnlyShortcut, !other.isMouseShortcut
        {
            return self == other
        }
    ...
    public var isModifierOnlyShortcut: Bool {
        self.modifierTriggerFlag != nil
    }

    public var expectedModifierFlags: NSEvent.ModifierFlags? {
    ...
    public func matchesMouse(button: Int, modifiers: NSEvent.ModifierFlags) -> Bool {
        guard self.isMouseShortcut, let mouseButton else { return false }
    ...
    public static func == (lhs: HotkeyShortcut, rhs: HotkeyShortcut) -> Bool {
