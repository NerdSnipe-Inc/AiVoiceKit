// Sources/AiVoiceKit/Shared/FileLogger.swift
// Ported from FluidVoice/Sources/Fluid/Services/FileLogger.swift
// Changes: public access, log directory/file names changed to AiVoiceKit.
import Darwin
import Foundation
...
public final class FileLogger: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.nerdsnipe.alric.voice.file.logger", qos: .utility)
    ...
    private nonisolated(unsafe) static var crashFileDescriptor: Int32 = -1
    private nonisolated(unsafe) static var crashHandlersInstalled = false
    ...
    private init() {
        ...
        self.logDirectory = baseDirectory.appendingPathComponent("Logs/AiVoiceKit", isDirectory: true)
        self.logFileURL = self.logDirectory.appendingPathComponent("AiVoiceKit.log", isDirectory: false)
        self.backupLogURL = self.logDirectory.appendingPathComponent("AiVoiceKit.log.1", isDirectory: false)
        // Legacy paths (Fluid-era) for one-time migration.
    ...
    public func appendSync(line: String) {
        self.queue.sync {
            self.appendUnlocked(line: line)
        }
    }
    ...
    private func createLogDirectoryIfNeeded() {
        ...
        do {
            ...
            // If the directory cannot be created, fall back to no-op.
        }
    }
    ...
    private func installCrashHandlersIfNeeded() {
        ...
        signal(SIGABRT, alricVoiceKitSignalHandler)
        signal(SIGILL, alricVoiceKitSignalHandler)
        signal(SIGSEGV, alricVoiceKitSignalHandler)
        signal(SIGBUS, alricVoiceKitSignalHandler)
        signal(SIGTRAP, alricVoiceKitSignalHandler)
    }

    public static func writeCrashSignalLine(_ signalNumber: Int32) {
    ...
    public static func writeCrashLineDirect(_ line: String) {
        guard self.crashFileDescriptor >= 0 else { return }
...
private func alricVoiceKitSignalHandler(_ signalNumber: Int32) {
